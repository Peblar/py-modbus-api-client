...
class Modbus():
    ...
    def write(self, addr: ModbusAddress | ModbusAddresses, data: Any):
        ...
        if addr.size != 1:  # If only single register is written
            prepended = ([0] * (addr.size - len(encoded_data))) + encoded_data
            resp = self._c.write_multiple_registers(addr.address, prepended)
        else:
            resp = self._c.write_single_register(addr.address, encoded_data)
